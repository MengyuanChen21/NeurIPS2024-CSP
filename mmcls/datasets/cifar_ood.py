...
from .ood_dataset import OODBaseDataset


@DATASETS.register_module()
class CIFAR10OOD(OODBaseDataset):
    ...
    def parse_datainfo(self):
        for idx, (sample, label) in enumerate(zip(self.imgs, self.gt_labels)):
            info = dict(img_prefix=self.base_folder)
            info['img_info'] = {'filename': '{}.jpg'.format(idx)}
            info['filename'] = '{}.jpg'.format(idx)
            info['type'] = 3  # no type
            info['label'] = int(label)  # no label
            info['img'] = Image.fromarray(sample)
    ...
    def prepare_data(self, idx):
        ...
        if self.aug is not None:
            print("[Warning] CIFAR dataset does not support augmentations.")
